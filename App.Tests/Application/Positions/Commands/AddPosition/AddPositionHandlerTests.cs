...
public class AddPositionHandlerTests
{
    [Fact]
        public async Task Handle_ReturnsOk_WithResultMappedFromDomain()
        {
            ...
            writer.Setup(w => w.AddAsync(It.IsAny<Position>(), It.IsAny<CancellationToken>()))
                  .Returns(Task.CompletedTask);

            writer.Setup(w => w.SaveChangesAsync(It.IsAny<CancellationToken>()))
                  .ReturnsAsync(1);

            var handler = new AddPositionHandler(writer.Object);
            ...
            value.Id.Should().NotBeEmpty();           // from domain entity
        ...
        public async Task Handle_CallsWriterWithPositionMappedFromCommand()
        {
            ...
            captured!.Id.Should().NotBeEmpty();           // created by ToDomain()
